...
class HashFile(BinaryFile):
    def __init__(
        ...
        super().__init__(
            filename, record, blocking_factor, empty_record, empty_record["id"]
        )
    ...
    def _insert_in_empty_bucket(
        self, file: BinaryIO, record: Dict
    ) -> None | Tuple[int, int]:
        file.seek(-self.L_size, 2)
        L = self.read_L(file)
        if L == -1:
    ...
    def insert(self, record: Dict):
        ...
        with open(self.filename, "rb+") as file:
            ...
            # Uvežemo u listu
            last = self._find_last_synonym(
                file, header["firstBucket"], header["firstIndex"]
            )
            if not last:
                return None
            ...
            if not block:
                return None
            block[1][last[1]].update(
                {"nextBucket": inserted_bucket, "nextIndex": inserted_index}
            )
            file.seek(-self.block_size, 1)
            self.write_block(file, *block)
            return inserted_bucket, inserted_index
    ...
    def print(self, detailed=False):
        with open(self.filename, "rb") as file:
            for i in range(self.B):
                ...
                print(
                    f"{header['previousFree'] if header['previousFree'] != -1 else '*'} ",
                    end="",
                )
                print(
                    f"{header['nextFree'] if header['nextFree'] != -1 else '*'} ",
                    end="",
                )
